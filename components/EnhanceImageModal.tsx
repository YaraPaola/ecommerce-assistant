        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-7xl transform transition-all h-[95vh] flex flex-col">
                ...
                <div className="flex-grow p-4 grid grid-cols-1 lg:grid-cols-4 gap-4 overflow-hidden min-h-0">
                    ...
                    {/* AI Settings Column */}
                    <div className="lg:col-span-1 space-y-3 overflow-y-auto flex flex-col h-full">
                            ...
                            <div className="grid grid-cols-1 gap-1.5">
                                {SIZES.map(size => (
                                    <button key={size.id} onClick={() => setSelectedSize(size.id)}
                                        className={`px-2 py-1.5 text-xs font-medium rounded transition-colors ${selectedSize === size.id ? 'bg-primary-accent text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                        {size.name}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="flex-shrink-0">
                            ...
                            <div className="grid grid-cols-1 gap-1 max-h-56 overflow-y-auto border rounded p-2 bg-white">
                                {backgroundPresets.map(preset => (
                                    <button key={preset.name} onClick={() => setCustomPrompt(preset.prompt)}
                                        className={`px-2 py-1.5 text-xs font-medium rounded transition-colors text-left ${customPrompt === preset.prompt ? 'bg-primary-accent text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}>
                                        {preset.name}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                            <h4 className="text-sm font-semibold text-gray-800 mb-2 flex-shrink-0">Custom Prompt</h4>
                            <textarea id="custom-prompt" value={customPrompt}
                                onChange={(e) => setCustomPrompt(e.target.value)}
                                placeholder="A sunlit, minimalist studio with a marble countertop..."
                                className="flex-1 block w-full rounded border-gray-300 shadow-sm focus:ring-primary-accent focus:border-primary-accent text-xs p-2 resize-none"
                            ></textarea>
                        </div>
                    </div>
                </div>
                <div className="px-4 py-3 bg-gray-50 flex justify-end border-t flex-shrink-0">
